...
Длина маршрута: 19
(одна единица длины маршрута примерно равна обычной длине одной ячейки)
...
Этот файл характеризует начальное состояние склада.
шаг - цикл прихода-ухода всех товаров(примерно день)
материал - порядковый номер материала (нумерация с нуля до количество материалов минус 1)

приход, уход и остаток измеряются в специфических единицах ёмкости склада - это число паллетов * 100.
А именно, если мы разделим это число ёмкости на 100, то получим дробную паллетную емкость(например, 2.5 паллета).
здесь остаток 2000 означает, что у нас по результату прихода-ухода занято 20 паллетов данным материалом.
а свободная паллетная ёмкость 40000 - это 400 свободных паллетов на складе
